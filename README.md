...
A daily push notification summarizes overnight activity: catalog movement (additions, removals, promotions), customer events (new trials, conversions, cancellations), and pipeline health (validation status, enrichment batches). Sections with no activity are omitted.

Instant alerts fire separately for urgent events — payment failures, refunds requiring action, catalog anomalies, and model resurrections. These arrive immediately, not batched.

### Remote configuration reduces release pressure

A remote configuration layer propagates changes to all users without a plugin release. What's updatable remotely:

- **Error documentation and message copy** — new error types get targeted, context-aware copy without a reinstall
- **Feature flags** — gate features behind a remote toggle; enable gradually, roll back instantly
- **Featured models** — update search suggestions when noteworthy models launch
- **Incident banners** — push platform warnings with severity level and custom copy
- **Kill switches** — disable specific features for emergency rollback

All remote files are integrity-verified before being applied. Tampered or corrupted files are rejected silently; the plugin falls back to built-in defaults.

### Self-learning systems improve with usage
...
**Generation time estimates.** Successful generations record their duration. After a few samples per model, a time estimate appears on the model card — derived from actual usage, not published benchmarks.
